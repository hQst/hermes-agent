...
class TestProfileHeader:
    """Tests for the X-Hermes-Profile header feature.

    Profile switching lets a single Hermes instance serve requests for
    multiple profiles.  Security: requires API key auth (same model as
    X-Hermes-Session-Id).
    """

    # -- Unit tests for _resolve_profile_from_request (no server needed) --

    def test_resolve_profile_empty_header(self, adapter):
        """Empty or missing X-Hermes-Profile returns None."""
        mock_request = MagicMock()
        mock_request.headers.get.return_value = ""
        assert APIServerAdapter._resolve_profile_from_request(mock_request) is None

    def test_resolve_profile_valid_name(self, adapter):
        """Valid profile name is returned as-is."""
        mock_request = MagicMock()
        mock_request.headers.get.return_value = "my-profile"
        assert (
            APIServerAdapter._resolve_profile_from_request(mock_request) == "my-profile"
        )

    def test_resolve_profile_simple_name(self, adapter):
        """Simple alphanumeric profile name is accepted."""
        mock_request = MagicMock()
        mock_request.headers.get.return_value = "coder"
        assert APIServerAdapter._resolve_profile_from_request(mock_request) == "coder"

    def test_resolve_profile_uppercase_rejected(self, adapter):
        """Profile names with uppercase characters are rejected (returns None)."""
        mock_request = MagicMock()
        mock_request.headers.get.return_value = "MyProfile"
        assert APIServerAdapter._resolve_profile_from_request(mock_request) is None

    def test_resolve_profile_special_chars_rejected(self, adapter):
        """Profile names with special characters are rejected (returns None)."""
        mock_request = MagicMock()
        mock_request.headers.get.return_value = "my-profile!"
        assert APIServerAdapter._resolve_profile_from_request(mock_request) is None

    def test_resolve_profile_control_chars_rejected(self, adapter):
        """Profile names with control characters are rejected."""
        mock_request = MagicMock()
        mock_request.headers.get.return_value = "pro\rfile"
        assert APIServerAdapter._resolve_profile_from_request(mock_request) is None

    def test_resolve_profile_too_long_rejected(self, adapter):
        """Profile names longer than 64 chars are rejected."""
        mock_request = MagicMock()
        mock_request.headers.get.return_value = "a" * 65
        assert APIServerAdapter._resolve_profile_from_request(mock_request) is None

    # -- Integration tests via HTTP --

    @pytest.mark.asyncio
    async def test_profile_without_api_key_returns_403(self, adapter):
        """Profile switching requires API key authentication."""
        app = _create_app(adapter)
        async with TestClient(TestServer(app)) as cli:
            resp = await cli.post(
                "/v1/chat/completions",
                headers={"X-Hermes-Profile": "coder"},
                json={
                    "model": "hermes-agent",
                    "messages": [{"role": "user", "content": "Hi"}],
                },
    ...
    @pytest.mark.asyncio
    async def test_profile_nonexistent_returns_404(self, auth_adapter):
        """Requesting a profile that doesn't exist returns 404."""
        app = _create_app(auth_adapter)
        async with TestClient(TestServer(app)) as cli:
    ...
    @pytest.mark.asyncio
    async def test_profile_passed_to_run_agent(self, auth_adapter):
        """When X-Hermes-Profile is provided, it's forwarded to _run_agent."""
        mock_result = {
            "final_response": "Hello from coder!",
            "messages": [],
            "api_calls": 1,
        }
        app = _create_app(auth_adapter)
        async with TestClient(TestServer(app)) as cli:
            ...
            assert resp.status == 200
            call_kwargs = mock_run.call_args.kwargs
            assert call_kwargs.get("profile") == "coder"

    @pytest.mark.asyncio
    async def test_no_profile_header_uses_active_profile(self, auth_adapter):
        """Without X-Hermes-Profile, profile is None (active profile used)."""
        mock_result = {"final_response": "Hello!", "messages": [], "api_calls": 1}
        app = _create_app(auth_adapter)
        async with TestClient(TestServer(app)) as cli:
            with patch.object(
    ...
    @pytest.mark.asyncio
    async def test_profile_responses_endpoint(self, auth_adapter):
        """X-Hermes-Profile works with /v1/responses endpoint too."""
        mock_result = {"final_response": "Hello!", "messages": [], "api_calls": 1}
        app = _create_app(auth_adapter)
        async with TestClient(TestServer(app)) as cli:
            with (
    ...
    @pytest.mark.asyncio
    async def test_invalid_profile_name_falls_back_to_active(self, auth_adapter):
        """Invalid profile name in header is silently ignored, active profile used."""
        mock_result = {"final_response": "Hello!", "messages": [], "api_calls": 1}
        app = _create_app(auth_adapter)
        async with TestClient(TestServer(app)) as cli:
            with patch.object(
            ...
            # _resolve_profile_from_request returns None for invalid names,
            # so the request proceeds with the active profile (no profile header).
            assert resp.status == 200
            call_kwargs = mock_run.call_args.kwargs
            assert call_kwargs.get("profile") is None
