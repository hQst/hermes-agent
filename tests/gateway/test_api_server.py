...
# ---------------------------------------------------------------------------
# X-Hermes-Profile header (profile switching)
# ---------------------------------------------------------------------------


class TestProfileHeader:
    @pytest.mark.asyncio
    async def test_profile_header_without_api_key_returns_403(self, adapter):
        """Profile switching requires API key authentication."""
        app = _create_app(adapter)
        async with TestClient(TestServer(app)) as cli:
            resp = await cli.post(
                "/v1/chat/completions",
                headers={"X-Hermes-Profile": "some-profile"},
                json={"model": "hermes-agent", "messages": [{"role": "user", "content": "Hi"}]},
            )
            assert resp.status == 403
            data = await resp.json()
            assert "Profile switching requires API key" in data["error"]["message"]

    @pytest.mark.asyncio
    async def test_profile_header_with_invalid_profile_returns_404(self, auth_adapter):
        ...
        async with TestClient(TestServer(app)) as cli:
            resp = await cli.post(
                "/v1/chat/completions",
                headers={
                    "X-Hermes-Profile": "nonexistent-profile",
    ...
    @pytest.mark.asyncio
    async def test_profile_header_passed_to_run_agent_chat_completions(self, auth_adapter):
        """Valid profile header is passed to _run_agent for chat completions."""
        mock_result = {"final_response": "Hello!", "messages": [], "api_calls": 1}
        app = _create_app(auth_adapter)
        async with TestClient(TestServer(app)) as cli:
            with patch.object(auth_adapter, "_run_agent", new_callable=AsyncMock) as mock_run:
                mock_run.return_value = (mock_result, {"input_tokens": 0, "output_tokens": 0, "total_tokens": 0})
                with patch("gateway.platforms.api_server.APIServerAdapter._resolve_profile_from_request", return_value="test-profile"):
                    resp = await cli.post(
                        "/v1/chat/completions",
                        headers={
                            "X-Hermes-Profile": "test-profile",
                            "Authorization": "Bearer sk-secret",
                        },
                        json={"model": "hermes-agent", "messages": [{"role": "user", "content": "Hi"}]},
                    )
    ...
    @pytest.mark.asyncio
    async def test_profile_header_passed_to_run_agent_responses(self, auth_adapter):
        """Valid profile header is passed to _run_agent for responses."""
        mock_result = {"final_response": "Hello!", "messages": [], "api_calls": 1}
        app = _create_app(auth_adapter)
        async with TestClient(TestServer(app)) as cli:
            with patch.object(auth_adapter, "_run_agent", new_callable=AsyncMock) as mock_run:
    ...
    @pytest.mark.asyncio
    async def test_profile_header_passed_to_run_agent_runs(self, auth_adapter):
        """Valid profile header is passed via _create_agent for runs."""
        mock_result = {"final_response": "Hello!", "messages": [], "api_calls": 1}
        app = _create_app(auth_adapter)
        async with TestClient(TestServer(app)) as cli:
            ...
            # Patch _create_agent on the adapter instance directly
            original_create_agent = auth_adapter._create_agent
            auth_adapter._create_agent = _mock_create_agent
            try:
                with patch("gateway.platforms.api_server.APIServerAdapter._resolve_profile_from_request", return_value="test-profile"):
                    resp = await cli.post(
                        "/v1/runs",
                ...
                assert call_kwargs_captured.get("profile") == "test-profile"
            finally:
                auth_adapter._create_agent = original_create_agent

    @pytest.mark.asyncio
    async def test_profile_header_models_endpoint(self, auth_adapter):
        """Profile header works on models endpoint."""
        app = _create_app(auth_adapter)
        async with TestClient(TestServer(app)) as cli:
            with patch("gateway.platforms.api_server.APIServerAdapter._resolve_profile_from_request", return_value="test-profile"):
                resp = await cli.get(
    ...
    @pytest.mark.asyncio
    async def test_no_profile_header_uses_default(self, auth_adapter):
        """Without profile header, no profile is passed."""
        mock_result = {"final_response": "Hello!", "messages": [], "api_calls": 1}
        app = _create_app(auth_adapter)
        async with TestClient(TestServer(app)) as cli:
            with patch.object(auth_adapter, "_run_agent", new_callable=AsyncMock) as mock_run:
    ...
    @pytest.mark.asyncio
    async def test_profile_header_empty_uses_default(self, auth_adapter):
        """Empty profile header is treated as no profile."""
        mock_result = {"final_response": "Hello!", "messages": [], "api_calls": 1}
        app = _create_app(auth_adapter)
        async with TestClient(TestServer(app)) as cli:
            with patch.object(auth_adapter, "_run_agent", new_callable=AsyncMock) as mock_run:
                mock_run.return_value = (mock_result, {"input_tokens": 0, "output_tokens": 0, "total_tokens": 0})
                with patch("gateway.platforms.api_server.APIServerAdapter._resolve_profile_from_request", return_value=None):
                    resp = await cli.post(
                        "/v1/chat/completions",
                        headers={
                            "X-Hermes-Profile": "",
    ...
    @pytest.mark.asyncio
    async def test_profile_header_whitespace_only_uses_default(self, auth_adapter):
        """Whitespace-only profile header is treated as no profile."""
        mock_result = {"final_response": "Hello!", "messages": [], "api_calls": 1}
        app = _create_app(auth_adapter)
        async with TestClient(TestServer(app)) as cli:
            with patch.object(auth_adapter, "_run_agent", new_callable=AsyncMock) as mock_run:
    ...
    @pytest.mark.asyncio
    async def test_profile_header_streaming_chat_completions(self, auth_adapter):
        """Profile header works with streaming chat completions."""
        app = _create_app(auth_adapter)
        async with TestClient(TestServer(app)) as cli:
            async def _mock_run_agent(**kwargs):
                cb = kwargs.get("stream_delta_callback")
    ...
    @pytest.mark.asyncio
    async def test_profile_header_streaming_responses(self, auth_adapter):
        """Profile header works with streaming responses."""
        app = _create_app(auth_adapter)
        async with TestClient(TestServer(app)) as cli:
            async def _mock_run_agent(**kwargs):
                cb = kwargs.get("stream_delta_callback")
    ...
    @pytest.mark.asyncio
    async def test_profile_header_create_agent_receives_profile(self, auth_adapter):
        """_create_agent is called with the profile parameter."""
        mock_result = {"final_response": "Hello!", "messages": [], "api_calls": 1}
        app = _create_app(auth_adapter)
        async with TestClient(TestServer(app)) as cli:
